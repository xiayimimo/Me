		...
		<div id="tabledata">
			<table class="table">
				<tr>
					...
					<th>问题名称</th>
					<th>发布者</th>
					<th class="timedata">发布时间</th>
					<th>操作</th>
				</tr>
				<c:forEach items="${sessionScope.questionList}" var="question" varStatus="s">
					<tr id="${s.count}">
						<td>${s.count}</td>
						<td><a href="getAnswer.do?id=${question.getId()}" target="content">${question.getTitle()}</a></td>
						<td>
						...
						<td><a class="del" href="javascript:void(0);" onclick="delQuestion('${question.getId() }',this)">删除</a></td>
					</tr>
				</c:forEach>
			</table>
			<table>
				<tr>
					<td class="pagetools">
						<form action="page.do" id="pageform" method="post">
...
<div class="page">
		...
		<c:if test="${sessionScope.currentPage ne 1}">
			<a href="" class="changePage">上一页</a>
		</c:if>
		<!-- 判断本页的起始位置如果大于0,，即不为第1页,显示可以单击上一页 -->
				<c:forEach begin="1" end="${sessionScope.pageCount}" var="p">
				<c:if test="${sessionScope.currentPage eq p}">
				 <span class="page-curr" >
						<em class="page-em" style="background-color:#1E9FFF;"></em>
						<em id ="currentPage">${p}</em>
					</span> 
				</c:if>
				<c:if test="${sessionScope.currentPage ne p}">
					<a href="" class="page-em">${p}</a>
				</c:if>	
				</c:forEach>
		<!-- 判断本页的起始位置如果为最后一页,直接显示下一页 -->
		
		<!-- 判断本页的起始位置如果不为最后一页,显示可以单击下一页 -->
		<c:if test="${sessionScope.currentPage eq sessionScope.pageCount}">
			<a href="#" >下一页</a>
		</c:if>
		<c:if test="${sessionScope.currentPage lt sessionScope.pageCount}">
			<a href="" class="changePage">下一页</a>
		</c:if>
</div>
